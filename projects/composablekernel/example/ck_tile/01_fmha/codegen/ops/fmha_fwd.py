...
bool get_num_cus(unsigned& num_cus) {
    ...
    // hipDeviceProp_t::multiProcessorCount reports WGP count on gfx11 (RDNA3).
    // Most heuristics in this file assume CU count, so convert WGP -> CU on gfx11.
    if(std::strncmp(props.gcnArchName, "gfx11", 5) == 0) {
        num_cus *= 2;
    }
    return true;
}
...
class KernelComponentFactoryGfx11(CompatibilityRuleFactory):
    ...
    def get_rules(cls) -> List[CompatibilityRule]:
        ...
        def check_pad_tile(problem_ctx: ProblemContext, kernel_ctx: KernelContext) -> bool:
            # gfx11 (wave32 WMMA): no-pad 128x128 tends to spill more than padded variants.
            if (problem_ctx.hdim, problem_ctx.hdim_v) != (128, 128):
                return True
            if problem_ctx.dtype not in cls._DT_FP16_BF16:
                return True

            if kernel_ctx.tile.F_bm0 != 128 or kernel_ctx.tile.F_bn0 != 128:
                return True

            is_npad = (
                kernel_ctx.pipeline.F_spad == "f"
                and kernel_ctx.pipeline.F_skpad == "f"
                and kernel_ctx.pipeline.F_dpad == "f"
                and kernel_ctx.pipeline.F_dvpad == "f"
            )
            if is_npad:
                return False
    ...
    def get_hdim_tile_size_dict(cls, dtype: str) -> Optional[dict]:
        if dtype in cls._DT_FP16_BF16:
            return {
                ...
                ( 32,  32) : [FmhaFwdTileSize( 64,  64,  16,  32,  32,   32,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1)],
                ( 64,  64) : [FmhaFwdTileSize( 64,  64,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1)],
                # Prefer 128x128 on longer seqlens; 64x64 is a better default for shorter seqlens.
                (128, 128) : [
                    ...
                    FmhaFwdTileSize(128,  64,  32, 128,  32,  128,  8, 1, 1,  8, 1, 1,  16, 16, 16,  16, 16, 16,  -1,

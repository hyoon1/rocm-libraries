...
inline std::optional<std::string> find_matching_kfd_node(const kfd_device_location& loc)
{
    ...
    while(auto* ent = readdir(dir))
    {
        ...
        const std::string prop_path =
            std::string(kKfdNodesDir) + "/" + node_name + "/properties";
...
{
    // Enable grouping on gfx11/gfx12 paths.
                           ...
                           RunKernelFn&& run_kernel_fn)
{
    ...
    for(ck_tile::index_t head_start = 0; head_start < nhead; head_start += group_sz)
    {
        ...
        args.k_ptr = ptr_offset<KDataType>(base_args.k_ptr, k_head_start * base_args.nhead_stride_k);
        args.v_ptr = ptr_offset<VDataType>(base_args.v_ptr, k_head_start * base_args.nhead_stride_v);
        args.o_ptr = ptr_offset<ODataType>(base_args.o_ptr, head_start * base_args.nhead_stride_o);

        args.bias_ptr = ptr_offset<BiasDataType>(base_args.bias_ptr,
                                                 head_start * base_args.nhead_stride_bias);
        args.lse_ptr =
        ...
        if(log_enabled())
        {
            ...
            std::cout << "[LLC Head Grouping] group " << (head_start / group_sz) << "/"
